...
The **Microcode panel** (top bar > Microcode) shows the 16x8 control-word table the machine executes - live, with the currently executing microstep glowing. Click a cell and toggle signals to edit; the running machine switches to your table immediately, modified cells highlight with per-word revert, and edits persist per machine. **Run examples** assembles and executes every example program against expected outputs on your edited microcode, and **Export EEPROM .bin** downloads the two 1KB images the Arduino programmer burns - so MSAP-2 instructions can be designed, validated, and only then flashed. The JC/JZ flag-bank patches are applied automatically on top of the template. Opcode rows are editable too: click a mnemonic to rename it or to name one of the free slots (1100/1101) with an operand shape - the in-browser assembler, disassembler, and analyzer pick the new ISA up immediately, and **Export .isa** writes a file that `msap-asm --isa-file custom.isa` uses for the real SD-card flow. Renaming an op that the bundled examples use means those examples won't assemble until reverted.

## Machines

The simulator is structured as machine plugins (`src/machines/`): a `MachineDefinition` bundles the ISA, control signals, inspector registers, board registry, examples, and microcode; MSAP-1 is the first entry, and MSAP-2 slots in as a sibling definition with a machine picker appearing automatically once a second machine exists. Per-machine state (design, probes, view settings, microcode) is namespaced in storage.
